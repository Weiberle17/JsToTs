            ...
            selected?.fields?.map((field, fidx) => {
              ...
              return (
                <Grid
                  item
                  key={field.key}
                  xs={!(field.endAdornment === '€') ? (field.size ?? 12) : 6}
                  sm={!(field.endAdornment === '€') ? (field.size ?? 12) : 3}
                  sx={{
                    padding: 1,
                    display: 'flex',
...
export const checkCompleted = (inputs) => {
  let complete = true
  Object.keys(inputs).forEach((key) => {
    if (inputs[key] && !inputs[key].isCompleted()) {
      complete = false
    }
  })
  return complete
...
export const applyAnswers = (inputs, data) => {
  Object.keys(inputs)?.forEach((key) => {
    inputs[key].setValues(data)
  })
}

// TODO: Weiter Inputmöglichkeiten hinzufügen? (Switch)
const Input = forwardRef(({field, disabled, onChange}, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    getValue: (prefix = '') => {
      const returnValue = {}
      const tempValue = (field.inputType === 'checkbox' ? (value ? "1" : "0") : value)
      if (tempValue) {
        returnValue[prefix + field.key] = inputTable.current?.getValue() ?? tempValue
        const refs = inputRefs.current
        const refKeys = Object.keys(refs)
        if (refKeys.length > 0) {
          refKeys.forEach((key) => {
            if (refs[key]?.getValue()) {
              Object.assign(returnValue, {...returnValue, ...refs[key].getValue(prefix)})
            }
          })
        }
      }
    ...
    setValues: (values) => {
      if (values) {
        ...
        Object.keys(values).filter(k => k.startsWith(field.key + '_')).forEach((key) => {
          inputRefs.current[key]?.setValues(values)
        })
      }
    },
    isCompleted: () => {
  ...
  const checkTextField = (newValue) => {
    ...
    if (field.type === 'email') {
      ...
      if (!!newValue && newValue.match(mailRegex)) {
        ...
        setError({error: true, errorText: 'Die E-Mail muss ein gülitges Format haben.'})
      }
  ...
  const inputProps = {
    ...
    field: field.field,
    value: value,
    checked: !!(typeof value === 'boolean' ? value : parseInt(value)),
  ...
  switch (field.inputType) {
    case 'table':
      // TODO: Anzeige von Tabellendaten
      return (
        <InputTable
          fields={field.fields}
          initRows={field.rows}
      ...
      return (
        <>
          ...
          {typeof value === 'string' && value.length > 0 && (
            <Link color="#F0AF00" href={value} rel="noreferrer" target="_blank">Hochgeladene Datei</Link>
          )}
          <input
